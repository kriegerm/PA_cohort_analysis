...
```{r}
result <- create_topic_model(counts_data, 5, TRUE, "Gibbs")
model <- result$lda_model
```

## Plot beta
```{r, fig.height=5, fig.width=15}
plot_beta(result, 4, result$b_df)
```
...
library(umap)
plot_gamma_umap(results, "Type", plot_colors, result$g_df)
```

##Membership of topic by Type
```{r, fig.height=3.5, fig.width=4}
topic_membership(result, "Type", plot_colors, result$g_df)
```


## Heatmap of rel abundance in original data of top taxa
```{r, fig.height=3, fig.width=7}
relab_heatmap(result, ps_obj_ped_fs, "Genus", "Type", 3, 5, result$b_df)
```

```{r, fig.height=3, fig.width=7}
relab_heatmap(result, ps_obj_ped_fs, "Genus", "Type", 2, 5, result$b_df)
```
